...
int  main()
{
	char fileName[] = "C:\\Users\\User\\Desktop\\week 15.txt";
	int i=0;
...
void printPerson(person p1)
{
	...
	printf("First Name:%s\nLast Name:%s\ne-mail:%s\nDOB:%d\nAge:%d\n\n", p1.firstName, p1.lastName, p1.email, p1.dob.year, getAge(p1.dob), getMonth(p1.dob.month));
}
...
date getDate(char *dateString)
{
	...
	char *token;
	int day, month, year;
	//year = (year[0] -48 )*1000 + (year[1]-48)*100 + (year[2]-48)*10;
	//year = atoi(token);
	
	// INSERT YOUR CODE HERE TO USE STRTOK TO CONVERT THE DATE STRING INTO A DATE STRUCTURE
	
		if ((token = strtok(dateString, delim)) != NULL) month=atoi(token);
		if ((token = strtok(NULL, delim)) != NULL) day=atoi(token);
